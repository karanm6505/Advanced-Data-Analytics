...
# -------------------------
t_lstm = np.arange(len(y_train) + len(y_test))

plt.figure(figsize=(12, 5))
plt.plot(t_lstm[:len(y_train)], y_train, label='Train Actual (LSTM)', color='blue')
plt.plot(t_lstm[len(y_train):], y_test, label='Test Actual (LSTM)', color='green')
plt.plot(t_lstm[:len(predicted_train)], predicted_train, label='Train Predicted (LSTM)', color='red', linestyle='--')
plt.plot(t_lstm[len(y_train):len(y_train)+len(predicted_test)], predicted_test, label='Test Predicted (LSTM)', color='orange', linestyle='--')
